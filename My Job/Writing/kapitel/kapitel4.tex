...
After moving average time series we are interested in looking for the abrupt changing point in the timeseries. By achieving that there are a several ways. For example, we can find the points where there is a abrupt change in the mean, variance or slope and intercept of the corresponding elements of the series. In this work, the abrupt changes in mean are enough. This can be achieved using MatLab function \textit{ischange}, we can choose to set different maximum number of change points. We have tried to set this number from 2 to 4, in each case, only 2 changing points can be found.
