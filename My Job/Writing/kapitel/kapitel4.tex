...
\subsection{Validation of model-based estimations}\label{sec:error}
...
\section{Estimating the runoff using quantile function and water level height}\label{sec:waterlevel}
Since the total water storage, the precipitation and evapotransipiration are available, the runoff can be determined using equation of terrestrial water balance \autoref{equa:waterbalence}. However, in \cite{10.1175/JHM-D-13-0157.1}, it was proved that this budget based approaches do not provide realistic runoff estimates because of significant biases in the input datasets. Therefore, other methods to determine runoff has to be considered. For example by using global runoff datasets or using quantile function with water level height. \\\\
